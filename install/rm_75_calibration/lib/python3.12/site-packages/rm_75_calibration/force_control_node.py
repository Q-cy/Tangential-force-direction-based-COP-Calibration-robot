...
class ForceControlNode(Node):
    def __init__(self):
        ...
        # ==================== 通用参数 ====================
        self.declare_parameter('control_axes', 'z')
        ...
        self.declare_parameter('trajectory_duration', 0.0)
        self.declare_parameter('hold_duration', 0.0)

        # ==================== 每轴独立参数 ====================
        for ax in ('x', 'y', 'z'):
            defaults = {
                'x': {'target': 5.0, 'step': 0.00005, 'approach': 0.0001,
                      'fine_ratio': 0.8, 'coarse_wait': 10, 'fine_wait': 50,
                      'drift': 0.2, 'step_sign': 1, 'start_delay': 0.0,
                      'force_field': 'force.x', 'force_sign': 1},
                'y': {'target': 5.0, 'step': 0.00005, 'approach': 0.0001,
                      'fine_ratio': 0.8, 'coarse_wait': 10, 'fine_wait': 50,
                      'drift': 0.2, 'step_sign': 1, 'start_delay': 0.0,
                      'force_field': 'force.y', 'force_sign': 1},
                'z': {'target': 10.0, 'step': 0.00005, 'approach': 0.0001,
                      'fine_ratio': 0.8, 'coarse_wait': 10, 'fine_wait': 50,
                      'drift': 0.2, 'step_sign': 1, 'start_delay': 0.0,
                      'force_field': 'force.z', 'force_sign': -1},
            }[ax]
            self.declare_parameter(f'{ax}_target_force', defaults['target'])
            self.declare_parameter(f'{ax}_step_size', defaults['step'])
            self.declare_parameter(f'{ax}_approach_step_size', defaults['approach'])
            self.declare_parameter(f'{ax}_fine_threshold_ratio', defaults['fine_ratio'])
            self.declare_parameter(f'{ax}_coarse_stabilize_cycles', defaults['coarse_wait'])
            self.declare_parameter(f'{ax}_fine_stabilize_cycles', defaults['fine_wait'])
            self.declare_parameter(f'{ax}_drift_threshold', defaults['drift'])
            self.declare_parameter(f'{ax}_step_sign', defaults['step_sign'])
            self.declare_parameter(f'{ax}_start_delay', defaults['start_delay'])
            self.declare_parameter(f'{ax}_force_field', defaults['force_field'])
            self.declare_parameter(f'{ax}_force_sign', defaults['force_sign'])
